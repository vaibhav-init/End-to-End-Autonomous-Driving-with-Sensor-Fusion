...
class Collision():
  ...
  def clean(self):
    if not hasattr(self, '_collision_sensor'):
      return
    ...
    try:
      # Check if sensor still exists and is alive before any operation
      if hasattr(self._collision_sensor, 'is_alive') and self._collision_sensor.is_alive:
        # Only stop if it's actually listening
        try:
          if hasattr(self._collision_sensor, 'is_listening') and self._collision_sensor.is_listening:
            self._collision_sensor.stop()
        except RuntimeError:
          # Sensor already stopped or can't be stopped
          pass
        ...
        try:
          ...
          # Sensor already destroyed
          pass
    except (RuntimeError, AttributeError):
      # Sensor object is invalid or already gone
      pass
    finally:
